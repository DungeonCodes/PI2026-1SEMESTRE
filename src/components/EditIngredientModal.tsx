...
const EditIngredientModal: React.FC<EditIngredientModalProps> = ({ isOpen, onClose, ingredient }) => {
  const { updateIngredient } = useStock();
  ...
  useEffect(() => {
    if (ingredient) {
      ...
      setMinQuantity(ingredient.quantidade_minima.toString());
      setUnit(ingredient.unidade_medida);
    }
  }, [ingredient]);

  const handleSubmit = async (e: React.FormEvent) => {
    e.preventDefault();
    if (ingredient && name && minQuantity && unit) {
      await updateIngredient(ingredient.id, name, parseInt(minQuantity, 10), unit);
      onClose();
    } else {
  ...
  return (
    <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50">
      <div className="bg-gray-800 p-6 rounded-lg shadow-xl w-full max-w-md">
        <h2 className="text-2xl font-bold mb-4 text-orange-500">Editar Ingrediente</h2>
